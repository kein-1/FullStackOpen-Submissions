...
loginRouter.post("/", async (request,response) => {
    ...
    if (person === undefined || person.length == 0) return response.status(401).send("User not found!") 
    ...
    const passwordCorrect = await bcrypt.compare(password,person[0].passwordHash)
